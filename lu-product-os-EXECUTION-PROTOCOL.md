...
### Before you start (every build session)
Session startup applies to **build work** — sessions that will edit code or config. Read-only sessions (questions, audits, analysis, planning discussion) need only the entry file; doctor and lessons are not a gate for reading, and their absence is not a blocker there.
...
16. **Stop.** Acceptance and merge are distinct authorities. **Acceptance authority is Lu's alone** — he decides whether the outcome is accepted. **Merge authority defaults to Lu** (squash merge); an agent may perform the mechanical merge only when Lu has explicitly delegated that authority AND every applicable gate is green — never to clear a red. **Promotion PRs (integration → default branch) always merge with `--merge`, never squash** — squashing mints a new SHA on the default branch and permanently forks the integration branch from it. Absent delegation: do not merge, do not nag; if everything is green, say so once.

### After merge (automation + light touches)
17. **Codebase docs regenerate post-merge** (Documentor Update), which writes the freshness stamp doctor checks at session start — `0-documentation-codebase/.docs-fresh-sha` in a single repo; in a multi-repo workspace, one stamp per stack at `<workspace>/0-documentation-codebase/<stack>/.docs-fresh-sha` holding that member repo's latest code commit (with `<stack>/.docs-repo` naming the member repo directory) — if the project has no automation for this, trigger the Update at the end of the session. If you changed business rules or domain concepts, update the product-intent docs in the same PR. **Flip the progress checkbox** in CLAUDE.md/AGENTS.md — a one-line inline edit to both files; that is the entire progress update.
